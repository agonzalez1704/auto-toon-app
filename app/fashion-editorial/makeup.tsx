...
export default function MakeupScreen() {
  ...
  const handlePick = useCallback(async () => {
    ...
    store.setMakeupRef({ localUri: uri, analysis: null, mode: null, phase: 'analyzing' })
    try {
      const base64 = await FileSystem.readAsStringAsync(uri, { encoding: 'base64' })
      const result = await analyzeMakeup(base64)
      store.updateMakeupRef({ analysis: result.analysis, mode: result.mode, phase: 'ready' })
    } catch (err: any) {
      store.updateMakeupRef({ phase: 'error', error: err?.message || 'Failed' })
    }
  }, [store])
    ...
    <View style={styles.root}>
      ...
      <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Makeup Reference</Text>
          <View style={{ width: 40 }} />
        </View>
        ...
        <View style={styles.content}>
          {!ref?.localUri ? (
            <>
              <Text style={styles.hint}>Upload a face photo with makeup or a makeup product image.</Text>
            ...
            <>
              <View style={styles.imageWrap}>
                ...
                {ref.phase === 'analyzing' && (
                  <View style={styles.overlay}>
                    ...
                    <Text style={styles.overlayText}>Analyzing makeup...</Text>
                  </View>
                )}
              </View>
              ...
              {ref.phase === 'ready' && (
                <View style={styles.analysisBox}>
                  ...
                  <Text style={styles.analysisText} numberOfLines={6}>{ref.analysis}</Text>
                </View>
              )}
              ...
              <View style={styles.actions}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => { store.setMakeupRef(null) }}
                  activeOpacity={0.7}
                >
